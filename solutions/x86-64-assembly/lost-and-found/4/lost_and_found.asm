...
;   [rsp + 8]: The address of a new entry to be added to the list.
; Note:
...
print_item:
    ...
    xor rdi, rdi        ; Arg 1: Clear rdi to 0, which will be used as the first argument for the printing function (the intro string)
    mov rsi, rdx        ; Arg 2: Store the index in rsi (rdi already holds the intro string arg unchanged — no mov needed)
    mov rdx, [r10]      ; Arg 3: Load the ID of the item into rdx
    mov rcx, [r10+8]    ; Arg 4: Load the description address of the item into rcx
    mov r8, [r10+16]    ; Arg 5: Load the day of the item into r8
    mov r9, [r10+24]    ; Arg 6: Load the month of the item into r9
    lea r11, [r10+40]   ; Arg 8: Load the address of the first category string into r11
    push r11            ; Push the address of the first category string onto the stack
    push qword [r10+32] ; Arg 7: Push the number of categories for the item onto the stack
